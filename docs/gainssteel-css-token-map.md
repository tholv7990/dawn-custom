...
6. **Beware the `.gs-scope` margin reset.** `assets/section-ct-base.css` zeroes `.gs-scope h1..h5` and `.gs-scope p` margins at specificity **0-1-1**. A section's own `.ct-x__heading` / `.ct-x__lead` (a class = 0-1-0) is OUTRANKED and its `margin` is silently stripped → headings/leads collapse together while sibling `<div>` blocks keep their margins (asymmetric vertical rhythm). Fix: prefix intro heading/paragraph rules with the section root class (`.ct-x .ct-x__heading` = 0-2-0) so the margin survives.
7. **Vertical rhythm: each stacked block carries margin-bottom ONLY (never margin-top).** Mixing `margin-bottom` on one block with `margin-top` on the next double-stacks the gap and breaks symmetry. (Open watch item from hero QA — verify no block pairs both a bottom and a top margin.)
8. **Solid vs outline icons.** Dawn's `icon-star.svg` is an *outline* star (evenodd ring) — filling it yields a gold outline, not a solid star. For a solid rating star, inline the design's single-path star SVG (`M12 2l3.1 6.3 …`) and set `fill: var(--ct-rating)`.
