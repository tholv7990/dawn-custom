...
| R4 | **Scope = landing page + PDP polish**. Homepage (`index.json`) is out of scope. | Shared sections (`ct-use-cases`, `ct-pricing`, `ct-reviews`) mean PDP polish is almost free once landing is done. |
| R5 | **Token block is unchanged**. `snippets/css-variables.liquid` already emits the CozyClaw purple values. Reskinning is **not** part of this work. | Already done in P5. The redesign is about *page composition* + *missing primitives*, not tokens. |
